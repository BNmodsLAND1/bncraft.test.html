...
Push **all files in this folder to the repo root** — not inside a `BNcraft/` subfolder — so `Mods.html` is at:

`https://bnmodsland1.github.io/bncraft.test.html/Mods.html`

```bash
cd path/to/bncraft.test.html
git add Mods.html index.html other-mods.html BNcraft-other-mods.html links.html tutorial.html css js
git commit -m "Update BNcraft site"
git push
```
